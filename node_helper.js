...
module.exports = NodeHelper.create(Object.assign({
        ...
        combineConfig: function() {
            // function copied from DiscoverITLLC (MIT)
            ...
            fs.readFile(path.resolve(__dirname + "/modules.json"), (err, data) => {
                ...
                for (let i = 0; i < defaultModules.length; i++) {
                    self.modulesAvailable.push({
                        ...
                        installed: true,
                        author: "DiscoverITLLC",
                        desc: "",
                        id: "DiscoverITLLC/MagicMirror",
                        url: "https://github.com/DiscoverITLLC/MagicMirror/wiki/MagicMirror%C2%B2-Modules#default-modules"
                    });
        ...
        answerConfigHelp: function(query, res) {
            if (defaultModules.indexOf(query.module) !== -1) {
                ...
                git.revparse(["HEAD"], function(error, result) {
                    ...
                    res.writeHead(302, { 'Location': "https://github.com/DiscoverITLLC/MagicMirror/tree/" + result.trim() + "/modules/default/" + query.module });
                    res.end();
                });
                return;
            }
